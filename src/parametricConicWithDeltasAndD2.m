...
%-------------------------------------------------------------------------------
function xy1= createRotatedParabolaWithJitter(theta, x0=-3, y0=10, from=-6, to=6)
...
%-------------------------------------------------------------------------------
%-------------------------------------------------------------------------------
function [xy1 deltas seconddeltas]= picktsGivenXY(XYs)
  ...
  seconddeltas=[];
  for i = 1:n-1
    deltax=XYs(i,1) - previousxy(1); 
    if(deltax != 0)
      deltay=XYs(i,2) - previousxy(2);
      deltas= [deltas deltay/deltax];
      if( length(deltas)>1 )
        d2=deltas(end) - deltas(end-1);
        seconddeltas= [seconddeltas d2];
      endif
    endif
    xy1(i,:)= [i i^2 1];
  endfor
...
for theta = [0 pi/12 pi/4 -pi/4 pi*3.25 -pi *2/3]
  ...
  [X deltas delta2s]= picktsGivenXY(rotated_tfeatures);
  % debug deltas,delta2s;
  ...
  %extrapolate it
  rangeXToExtrapolate = linspace(-10, 10, 21);
